...
class blog
{
    ...
    public function category(){
        $any_arr = [];
        $any_query = mysql_query("SELECT * FROM category WHERE type=2") or die("Can't connect");
        while ($any_fetch = mysql_fetch_assoc($any_query)) {
            array_push($any_arr, $any_fetch);
        }
        return $any_arr;
    ...
    public function blog_tags(){
    	$blogTag_query = mysql_query("SELECT * FROM blog_tags WHERE item_id='".$this->id()."'") or die("Can't connect");
    	$string = "";
    	while ($blogTag_fetch = mysql_fetch_array($blogTag_query)) {
    		$tag = $blogTag_fetch['tag_title'];
    		$string.=$tag.",";
    	}
